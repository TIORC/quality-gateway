-- ---------------------------------------------------------------------------
-- POPs — revisões, histórico de modificações, sugestões de melhoria e acesso
-- ---------------------------------------------------------------------------
-- Este arquivo:
--   1. acrescenta em `public.pops` os campos da nova ficha do POP —
--      `revisao`, `data_revisao`, `observacao_revisao`,
--      `setores_responsaveis` e `visualizadores` (quem pode visualizar);
--   2. cria `public.pop_revisoes` — histórico de modificações. Cada revisão
--      anterior fica arquivada com o snapshot completo do documento,
--      consultável apenas pela gestão (Administrador / Gestor da Qualidade);
--   3. cria `public.pop_sugestoes` — botão "Sugerir melhoria" do POP;
--   4. libera a decisão `lido` em `public.pop_leituras` (o botão "Lido"
--      substitui o antigo "Li e Concordo / Li e DISCORDO!");
--   5. avisa a Qualidade a cada sugestão de melhoria recebida;
--   6. publica as novas tabelas no Realtime.
--
-- IMPORTANTE — nenhum dado é apagado:
--   * nenhuma coluna antiga é removida e nenhum POP é excluído;
--   * os POPs já cadastrados entram como "Revisão 01", com a data de
--     aprovação (ou de criação) e com o setor atual já preenchido em
--     `setores_responsaveis` e `visualizadores`.
--
...
-- POPs já cadastrados continuam intactos: entram como Revisão 01 e, quando
-- ainda não há setor definido, o próprio setor atual do POP passa a ser o
-- responsável e o autorizado a visualizar.
update public.pops
set
  revisao = 1,
...
-- 4. Botão "Lido" (substitui "Li e Concordo / Li e DISCORDO!") ------------------
-- A decisão `lido` é SOMADA às decisões antigas: nada é removido e as leituras
-- já registradas (concordo/discordo) permanecem no banco para consulta.
...
-- 5. Aviso à Qualidade a cada sugestão de melhoria ------------------------------
...
-- 7. Realtime (revisões e sugestões) -------------------------------------------
